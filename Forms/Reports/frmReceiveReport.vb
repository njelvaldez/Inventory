...
Public Class frmReceiveReport
    Private RemoteDataSet As New DataSet
    Private ReceiveReport As String = System.Configuration.ConfigurationSettings.AppSettings.Item("ReceiveReport")
    ...
    Private Sub SetPath()
        ...
        txtTemplate.Text = ReceiveReport
        txtOutput.Text = ReceiveReport.Replace(".xlsx", currentdate + ".xlsx")
    End Sub

    Private Sub frmReceiveReport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub InventoryReportShow()
        Try
            ...
            BusinessObject.Sub_Show(ServerPath2, "ReceiveReportSelect", CommandType.StoredProcedure, RemoteDataSet, "ProductFormCT_Show")
    ...
    Private Sub ExportInventoryReport(ByVal strFileName As String)
        ...
        Try
            ...
            range = wssalestrend.Range("A2", "F" + GetRangeEnd(counter10k, reccount).ToString().Trim)
            Dim strArr(GetArraySize(counter10k, reccount), 5) As Object
            For Each dr As DataRow In dt.Rows
                'recno
                strArr(rownumber, 0) = dr("recno").ToString
                'htr.RecDate
                strArr(rownumber, 1) = dr("RecDate").ToString
                'dtr.ItemCode
                ...
                If rownumber >= n10k - 1 Then
                    ...
                    ReDim strArr(0, 0)
                    ReDim strArr(GetArraySize(counter10k, reccount), 5)
                    range = wssalestrend.Range("A" + (GetRangeStart(counter10k, reccount)).ToString().Trim(), "F" + GetRangeEnd(counter10k, reccount).ToString().Trim)
                    rownumber = 0
                Else
            ...
            ws.PivotTables("PivotTable1").SourceData = "Source!R1C1:R" + (reccount + 1).ToString.Trim + "C5"
            If reccount > 1 Then
