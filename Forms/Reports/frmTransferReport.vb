...
Public Class frmTransferReport
    ...
    Private TransferReport As String = System.Configuration.ConfigurationSettings.AppSettings.Item("TransferReport")
    ...
    Private Sub SetPath()
        ...
        txtTemplate.Text = TransferReport
        txtOutput.Text = TransferReport.Replace(".xlsx", currentdate + ".xlsx")
    End Sub

    Private Sub frmTransferReport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub InventoryReportShow()
        Try
            ...
            BusinessObject.Sub_Show(ServerPath2, "TransferReportSelect", CommandType.StoredProcedure, RemoteDataSet, "ProductFormCT_Show")
    ...
    Private Sub ExportInventoryReport(ByVal strFileName As String)
        ...
        Try
            ...
            range = wssalestrend.Range("A2", "H" + GetRangeEnd(counter10k, reccount).ToString().Trim)
            Dim strArr(GetArraySize(counter10k, reccount), 7) As Object
            For Each dr As DataRow In dt.Rows
                'trnno
                strArr(rownumber, 0) = dr("trnno").ToString
                'htr.TrnDate
                strArr(rownumber, 1) = dr("TrnDate").ToString
                'dtr.ItemCode
                strArr(rownumber, 2) = dr("ItemCode").ToString
                ' itm.ItemDesc
                strArr(rownumber, 3) = dr("ItemDesc").ToString
                'dtr.Qty 
                strArr(rownumber, 4) = dr("Qty").ToString
                'UpdateBy, 
                strArr(rownumber, 5) = dr("UpdateBy").ToString
                'dtr.FromLocCode 
                strArr(rownumber, 6) = dr("FromLocCode").ToString
                'ToLocCode, 
                strArr(rownumber, 7) = dr("ToLocCode").ToString
                'array is zero base , lets subtract by 1
                If rownumber >= n10k - 1 Then
                    range.Value2 = strArr
                    counter10k = counter10k + 1
                    ReDim strArr(0, 0)
                    ReDim strArr(GetArraySize(counter10k, reccount), 7)
                    range = wssalestrend.Range("A" + (GetRangeStart(counter10k, reccount)).ToString().Trim(), "H" + GetRangeEnd(counter10k, reccount).ToString().Trim)
                    rownumber = 0
                Else
            ...
            ws.PivotTables("PivotTable1").SourceData = "Source!R1C1:R" + (reccount + 1).ToString.Trim + "C7"
            If reccount > 1 Then
